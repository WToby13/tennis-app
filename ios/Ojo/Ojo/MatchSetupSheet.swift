...
struct PlayerEntry: Identifiable {
    ...
    /// An email invite. The address is kept for the invite; the *name* is a
    /// readable guess at it — "guillem.torner@x.com" becomes "Guillem Torner",
    /// not "guillem.torner" — because that name is what everyone sees on the
    /// match until they join. It's only a placeholder: their real profile name
    /// replaces it the moment the invite is claimed. The address is shown
    /// alongside it in the sheet, so you can check who you actually invited.
    static func invite(email: String) -> PlayerEntry {
        ...
        return PlayerEntry(name: name.isEmpty ? address : name, email: address)
    }
}
...
struct MatchSetup {
    ...
    func participants(mergedWith existing: [Participant]) -> [Participant] {
        var out: [Participant] = []
        for candidate in ([near, far] + others).compactMap(\.participant) + existing
        where !out.contains(where: { $0.isSamePerson(as: candidate) }) {
            ...
            ForEach(others) { person in
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 1) {
                        ...
                        // The address, not just the name guessed from it — so you
                        // can see exactly who the invite is going to.
                        if let email = person.email?.nonEmpty, !person.isLinked {
                            Text(email).font(.caption2).foregroundStyle(Theme.muted)
                        }
                    }
                    if person.isLinked {
...
private struct PersonField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // A player slot shows a name; when that name stands in for an email
            // invite, show the address underneath so it can be checked.
            if let email = entry.email?.nonEmpty, !entry.isLinked, !entry.name.equalsName(email) {
                Text(email).font(.caption2).foregroundStyle(Theme.muted)
            }
