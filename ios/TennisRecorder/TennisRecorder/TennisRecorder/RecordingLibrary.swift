...
final class RecordingLibrary: ObservableObject {
    ...
    init() {
        ...
        // An "uploading" recording with no active background job died between
        // launches — surface it as failed so it can be retried.
        var fixed = false
    ...
    /// Fetch the signed-in user's uploaded matches from the web and merge in any
    /// that aren't already represented locally (so uploads made on another device —
    /// or kept after this phone freed its local copy — still appear).
    func refreshFromCloud() async {
        guard let myId = await Supa.currentUserId(),
        ...
        cloud = videos.compactMap { v in
            guard v.ownerId?.lowercased() == myId else { return nil } // only mine
