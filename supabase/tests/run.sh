...
#
# Four suites: invites (set_participants, claim_invite, the signup trigger),
# moderation (blocking, reports, and what an account deletion leaves behind),
# notifications (who a comment's fan-out reaches, and who it must not), and
# events (who can read the usage record, and whether the GTM metrics are right).
# All four sets of rules live in SQL, so this is where they can actually be
# tested — no Supabase project and no network needed. Start a scratch cluster
# first:
#
...
# 0012 and 0020 need pg_cron, which a plain local Postgres doesn't have. Both
# only *schedule* work whose function is defined in an ordinary migration, so
# skipping them leaves nothing untested. (0018 is unrelated and does run.) Everything else runs, including
# 0014_moderation — it redefines get_feed, so a chain that skipped it would not
# be testing the function the app actually calls.
fail=0
for t in invites moderation notifications events; do
  echo "--- $t ---"
  ...
  for f in "$MIGRATIONS"/0*.sql; do
    case "$f" in *0012_*|*0020_*) continue;; esac
